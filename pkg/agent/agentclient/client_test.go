...
func TestServeData_HTTP(t *testing.T) {
	...
	if _, ok := client.conns[connID]; ok {
		t.Error("client.conns not released")
	}
	if _, ok := client.cleanups[connID]; ok {
		t.Error("client.cleanups not released")
	}
	if _, ok := client.dataChs[connID]; ok {
		t.Error("client.dataChs not released")
	}
